...
class multi_shallow_embedding(nn.Module):
    ...
    def __init__(self, num_nodes, k_neighs, num_graphs):
        ...
        # Attention weights
        self.attention_weights = Parameter(torch.Tensor(num_graphs, num_nodes, num_nodes))  # static attention Contribution #1
    ...
    def forward(self, device):
        ...
        attention_scores = softmax(self.attention_weights/ np.sqrt(self.num_nodes), dim=-1).to(device) # static attention Contribution #1
       
        # # Scale attention by the square root of the degree
        # degree = attention_scores.sum(dim=-1, keepdim=True).sqrt()
        scaled_attention = attention_scores 
