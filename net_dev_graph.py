...
class GNNStack(nn.Module):
    ...
    def __init__(self, gnn_model_type, num_layers, groups, pool_ratio, kern_size, 
        ...
        self.global_pool = nn.AdaptiveAvgPool2d((1,None))  #Saves the temporal dimension - Contribution #3
    ...
    def forward(self, inputs: Tensor):
        ...
        for tconv, gconv, bn, pool in zip(self.tconvs, self.gconvs, self.bns, self.diffpool):
            ...
            x, adj, w_adj = pool(x, adj)  # Apply temporal graph pooling, which might modify x and adj

            feature_time_maps.append(x.detach().cpu().numpy())  # Save feature representation
            adj_matrices.append(adj.detach().cpu().numpy())  # Save adjacency matrix after TG pooling
