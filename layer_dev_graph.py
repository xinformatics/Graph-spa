...
# MultiShallowEmbeddingEnhanced with Attention responsible for graph construction Contribution #1
class multi_shallow_embedding(nn.Module):
    ...
    def forward(self, device):
        ...
        attention_scores = softmax(self.attention_weights/ np.sqrt(self.num_nodes), dim=-1).to(device) # Contribution #2
...
class Dense_TimeDiffPool2d(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor):
        ...
        ################################################################### attention based extraction of weighted adjacency Contribution #2
        ...
        queries = self.query(out_features_flatpool)  # Shape: [B, G, N, N]
        keys = self.key(out_features_flatpool)  # Shape: [B, G, N, N]

        # Calculate attention scores
        attention_scores = torch.einsum('bgnl,bgml->bgnm', (queries, keys))  # [B, G, N, N]
